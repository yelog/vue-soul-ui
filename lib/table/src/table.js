...
var _default2 = {
    ...
    collectColumn: function collectColumn(value) {
      ...
      if (tableFullColumn.length) {
        ...
        if (tableFullColumn[cIndex].prop) {
          _tools.UtilTools.warn('s.error.delProp', ['prop', 'field']);
        }

        if (tableFullColumn[cIndex].label) {
          _tools.UtilTools.warn('s.error.delProp', ['label', 'title']);
        }
      }
  ...
  created: function created() {
    ...
    if (!_tools.UtilTools.getRowkey(this)) {
      _tools.UtilTools.error('s.error.emptyProp', ['row-id']);
    ...
    if (!_tableCore.default._edit && this.editConfig) {
      errorModuleName = 'Edit';
    } else if (!_tableCore.default._valid && this.editRules) {
      errorModuleName = 'Validator';
    } else if (!_tableCore.default._keyboard && (this.keyboardConfig || this.mouseConfig)) {
      errorModuleName = 'Keyboard';
    } else if (!_tableCore.default._resize && this.autoResize) {
      errorModuleName = 'Resize';
    }

    if (errorModuleName) {
      throw new Error(_tools.UtilTools.getLog('s.error.reqModule', [errorModuleName]));
    }
  ...
  render: function render(h) {
    ...
    !loading && !tableData.length ? h('div', {
      ...
      $table: this
    }, h) : _conf.default.i18n('s.table.emptyText'))]) : _e(),
    /**
     * 列宽线
     */
    isResizable ? h('div', {
