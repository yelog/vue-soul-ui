...
var Methods = {
  ...
  loadTableData: function loadTableData(datas, notRefresh) {
    ...
    if (scrollYLoad && !(height || maxHeight)) {
      _tools.UtilTools.error('s.error.scrollYReqProp', ['height | max-height']);
    }

    if (scrollYLoad && !showOverflow) {
      _tools.UtilTools.warn('s.error.scrollYReqProp', ['show-overflow']);
    }
  ...
  hasRowChange: function hasRowChange(row, field) {
    _tools.UtilTools.warn('s.error.delFunc', ['hasRowChange', 'isUpdateByRow']);
  ...
  getRecords: function getRecords() {
    _tools.UtilTools.warn('s.error.delFunc', ['getRecords', 'getData']);
  ...
  getAllRecords: function getAllRecords() {
    _tools.UtilTools.warn('s.error.delFunc', ['getAllRecords', 'getRecordset']);
  ...
  refreshColumn: function refreshColumn() {
    ...
    if (scrollXLoad) {
      if (this.isGroup) {
        _tools.UtilTools.warn('s.error.scrollXNotGroup');
      ...
      if (reserve) {
        if (value) {
          selectRows.forEach(function (row) {
            selectReserveRowMap[_tools.UtilTools.getRowid(_this23, row)] = row;
          });
        } else {
  ...
  hasRowExpand: function hasRowExpand(row) {
    _tools.UtilTools.warn('s.error.delFunc', ['hasRowExpand', 'isExpandByRow']);
  ...
  hasTreeExpand: function hasTreeExpand(row) {
    _tools.UtilTools.warn('s.error.delFunc', ['hasTreeExpand', 'isTreeExpandByRow']);
