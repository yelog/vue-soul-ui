  ...
  if (window.Blob) {
    ...
    if (opts.message !== false) {
      $table.$XModal.message({
        message: _conf.default.i18n('s.table.expSuccess'),
        status: 'success'
      });
    }
  } else {
    _tools.UtilTools.error('s.error.notExp');
  }
}
...
function handleImport($table, content, opts) {
  ...
  if (status) {
    ...
    if (opts.message !== false) {
      $table.$XModal.message({
        message: _conf.default.i18n('s.table.impSuccess'),
        status: 'success'
      });
    }
  } else if (opts.message !== false) {
    $table.$XModal.message({
      message: _conf.default.i18n('s.error.impFields'),
      status: 'error'
    });
  }
...
var _default = {
  methods: {
    ...
    _exportCsv: function _exportCsv(options) {
      _tools.UtilTools.warn('s.error.delFunc', ['exportCsv', 'exportData']);
    ...
    _exportData: function _exportData(options) {
      ...
      if (!_xeUtils.default.includes(_tableCore.default.exportTypes, opts.type)) {
        throw new Error(_tools.UtilTools.getLog('s.error.notType', [opts.type]));
      }
      ...
      if (!opts.original) {
        if (scrollXLoad || scrollYLoad) {
          ...
          _tools.UtilTools.warn('s.error.scrollOriginal');
        }
      }
    ...
    _importByFile: function _importByFile(file, opts) {
      ...
      if (window.FileReader) {
        ...
        var types = options.types || _tableCore.default.importTypes;
        ...
        if (_xeUtils.default.includes(types, type)) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onerror = function (e) {
              _tools.UtilTools.error('s.error.notType', [type]);
            };
        ...
        if (_xeUtils.default.includes(types, type)) {
          ...
          if (options.message !== false) {
            _this3.$XModal.message({
              message: _xeUtils.default.template(_conf.default.i18n('s.error.notType'), [type]),
              status: 'error'
            });
          }
