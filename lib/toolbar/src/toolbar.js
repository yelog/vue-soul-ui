...
var _default2 = {
  ...
  created: function created() {
    ...
    if (settingOpts.storage && !id) {
      return _tools.UtilTools.error('s.error.toolbarId');
    }

    if (!_tableCore.default._export && (this.export || this.import)) {
      _tools.UtilTools.error('s.error.reqModule', ['Export']);
    }
  ...
  render: function render(h) {
    ...
    }))])]) : null]), _tableCore.default._export ? h('s-import-panel', {
      props: {
    ...
    }) : _e(), _tableCore.default._export ? h('s-export-panel', {
      props: {
  ...
  methods: {
    ...
    loadStorage: function loadStorage() {
      ...
      if (refresh && !$grid) {
        if (!refreshOpts.query) {
          _tools.UtilTools.warn('s.error.notFunc', ['query']);
        }
      }
      ...
      if ($grid || $table) {
        ...
        if (resizable || setting) {
          throw new Error(_tools.UtilTools.getLog('s.error.barUnableLink'));
        }
      }
    ...
    hideColumn: function hideColumn(column) {
      _tools.UtilTools.warn('s.error.delFunc', ['hideColumn', 'table.hideColumn']);

      column.visible = false;
      return this.updateSetting();
    },
    showColumn: function showColumn(column) {
      _tools.UtilTools.warn('s.error.delFunc', ['showColumn', 'table.showColumn']);
    ...
    btnEvent: function btnEvent(evnt, item) {
      ...
      if (code) {
        if ($grid) {
          ...
          var btnMethod = _tableCore.Buttons.get(code);
    ...
    openExport: function openExport(options) {
      ...
      var types = defOpts.types || _tableCore.default.exportTypes; // 处理类型
