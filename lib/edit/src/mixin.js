...
var _default = {
  methods: {
    ...
    _insertAt: function _insertAt(records, row) {
      ...
      if (treeConfig) {
        throw new Error(_tools.UtilTools.getLog('s.error.noTree', ['insert']));
      }
    ...
    _remove: function _remove(rows) {
      ...
      if (treeConfig) {
        throw new Error(_tools.UtilTools.getLog('s.error.noTree', ['remove']));
      }
    ...
    _revert: function _revert() {
      _tools.UtilTools.warn('s.error.delFunc', ['revert', 'revertData']);
    ...
    _clearActived: function _clearActived(evnt) {
      ...
      actived.row = null;
      actived.column = null;
      return (_tableCore.default._valid ? this.clearValidate() : this.$nextTick()).then(this.recalculate);
    },
    ...
    _hasActiveRow: function _hasActiveRow(row) {
      _tools.UtilTools.warn('s.error.delFunc', ['hasActiveRow', 'isActiveByRow']);
